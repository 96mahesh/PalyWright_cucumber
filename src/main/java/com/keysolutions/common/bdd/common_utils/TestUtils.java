...
public class TestUtils {
	...
	public static String modifyXmlPayLoadFromDataTable(String request, List<Map<String, String>> table) {
		try {
			if (table.get(0).containsKey("Attribute")) {
				for (int i = 0; i < table.size(); i++) {

					if (table.get(i).get("Attribute").isEmpty())
						request = XmlUtils.updateElementValue(request, table.get(i).get("RequestFieldXPath"),
								table.get(i).get("Value"));
					else
						request = XmlUtils.updateAttributeValue(request, table.get(i).get("RequestFieldXPath"),
								table.get(i).get("Attribute"), table.get(i).get("Value"));
